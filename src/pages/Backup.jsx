...
export default function Backup() {
  ...
  useEffect(() => {
    const checkAndSendAutoBackup = async () => {
      if (!autoBackupEnabled || !autoBackupEmail) return;
  ...
  }, [autoBackupEnabled, autoBackupEmail]);

  const { data: allExpenses = [] } = useQuery({
    queryKey: ['expenses'],
    queryFn: () => base44.expenses.getAll(),
    initialData: [],
  });
  ...
  const sendAutoBackup = async () => {
    if (!autoBackupEnabled || !autoBackupEmail || isSending) return;

    setIsSending(true);
    
    try {
      // Use backend export and email functionality
      await base44.backup.email(autoBackupEmail, 'json', null);
      await base44.backup.email(autoBackupEmail, 'sql', null);

      localStorage.setItem('lastAutoBackupTime', new Date().getTime().toString());
      setSuccess('پاشەکەوتی ئۆتۆماتیک بە سەرکەوتوویی نێردرا');
  ...
  const sendBackupToEmail = async () => {
    ...
    try {
      ...
      // Use backend backup email functionality
      const month = selectedMonth === 'all' ? null : selectedMonth;
      await base44.backup.email(email, exportFormat, month);
  ...
  const resetDatabase = async () => {
    ...
    try {
      for (const expense of expenses) {
        await base44.expenses.delete(expense.id);
      }

      for (const driver of drivers) {
        await base44.drivers.delete(driver.id);
      }

      for (const employee of employees) {
        await base44.employees.delete(employee.id);
      }

      for (const type of expenseTypes) {
        await base44.expenseTypes.delete(type.id);
      }
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <Label htmlFor="auto-backup" className="text-base cursor-pointer">
                  چالاککردنی پاشەکەوتی ئۆتۆماتیکی ڕۆژانە (JSON + SQL)
                </Label>
              </div>
