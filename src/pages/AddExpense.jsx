...
export default function AddExpense() {
  ...
  const { data: allExpenses = [] } = useQuery({
    queryKey: ['expenses'],
    queryFn: () => base44.expenses.getAll('-created_date'),
    initialData: [],
  });
  ...
  const createExpenseMutation = useMutation({
    mutationFn: (data) => base44.expenses.create(data),
    onSuccess: () => {
  ...
  const createExpenseTypeMutation = useMutation({
    mutationFn: (data) => base44.expenseTypes.create(data),
    onSuccess: () => {
  ...
  const deleteExpenseTypeMutation = useMutation({
    mutationFn: (id) => base44.expenseTypes.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['expenseTypes'] });
    },
  });
  ...
  const handleDriverSelect = (driverId) => {
    ...
    if (driver) {
      const driverCurrency = driver.currency || 'IQD'; // Default to IQD if not set
                ...
                <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                  {filteredDrivers.map(driver => (
                    <button
                      ...
                      className={`
                        p-1.5 rounded-lg border-2 text-center font-medium transition-all duration-200
                ...
                <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2">
                  {expenseTypes.map(type => (
                    <button
                      key={type.id}
                      type="button"
                      onClick={() => handleChange('expense_type', type.type_name)}
                      className={`
                        p-2 rounded-lg border-2 text-center font-medium transition-all duration-200 text-xs
                      ...
                      <SelectContent>
                        {Object.keys(expenseTypeColors).map(color => (
                          <SelectItem key={color} value={color}>
                            <div className={`w-4 h-4 rounded ${expenseTypeColors[color]}`} />
                            {color}
                          </SelectItem>
                        ))}
                      </SelectContent>
