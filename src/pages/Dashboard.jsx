...
export default function Dashboard() {
  ...
  const { data: allExpenses = [], isLoading } = useQuery({
    queryKey: ['expenses'],
    queryFn: async () => {
      const data = await base44.expenses.getAll();
  ...
  const { data: allEmployees = [] } = useQuery({
    queryKey: ['employees'],
    queryFn: async () => {
      const data = await base44.employees.getAll();
      return Array.isArray(data) ? data : [];
    },
  });
  ...
  const totalMonthExpensesIQD = useMemo(() => {
    return currentMonthExpenses.filter(exp => exp.currency === 'IQD' || !exp.currency).reduce((sum, exp) => sum + Number(exp.amount || 0), 0);
  }, [currentMonthExpenses]);

  const totalMonthExpensesUSD = useMemo(() => {
    return currentMonthExpenses.filter(exp => exp.currency === 'USD').reduce((sum, exp) => sum + Number(exp.amount || 0), 0);
  }, [currentMonthExpenses]);

  const totalEmployeeSalaryIQD = useMemo(() => {
    return currentMonthEmployees.filter(emp => emp.currency === 'IQD').reduce((sum, emp) => sum + Number(emp.salary || 0), 0);
  }, [currentMonthEmployees]);

  const totalEmployeeSalaryUSD = useMemo(() => {
    return currentMonthEmployees.filter(emp => emp.currency === 'USD').reduce((sum, emp) => sum + Number(emp.salary || 0), 0);
  }, [currentMonthEmployees]);
  ...
  const unpaidAmountIQD = useMemo(() => {
    return unpaidExpenses.filter(exp => exp.currency === 'IQD' || !exp.currency).reduce((sum, exp) => sum + Number(exp.amount || 0), 0);
  }, [unpaidExpenses]);

  const unpaidAmountUSD = useMemo(() => {
    return unpaidExpenses.filter(exp => exp.currency === 'USD').reduce((sum, exp) => sum + Number(exp.amount || 0), 0);
  }, [unpaidExpenses]);
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-7 gap-2">
              {calendarDays.map((day, idx) => {
                ...
                const unpaidTotal = dayExpenses.filter(e => !e.is_paid).reduce((sum, e) => sum + Number(e.amount || 0), 0);
